...
static void dfs(TreeNode* root, int val, int depth, int* curDepth)
{
    if (!root) return;
    if (*curDepth == depth - 1)
    {
        ...
        TreeNode* newNodeLeft = new_node(val, oldLeft, NULL);
        TreeNode* newNodeRight = new_node(val, NULL, oldRight);
        root->left = newNodeLeft;
        root->right = newNodeRight;
        return;
    }
    ++(*curDepth);
    dfs(root->left, val, depth, curDepth);
    dfs(root->right, val, depth, curDepth);
    --(*curDepth);
}
